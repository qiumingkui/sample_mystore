...
public class CategoryTable extends Table<Category> {
	...
	private void init(){
		...
		// (Category object, ResultSet rs) -> object.changeXX(rs.getString(idColumn.name())));
		...
		descriptionColumn.changeResultSetSetter(
				(Category category, ResultSet rs) -> category.changeDescription(rs.getString(descriptionColumn.name())));
