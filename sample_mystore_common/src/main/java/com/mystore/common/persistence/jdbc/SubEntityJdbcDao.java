...
public abstract class SubEntityJdbcDao<T, ID, SUPID> extends JdbcBaseDao<T> {

	@SuppressWarnings("unchecked")
	public SubEntityJdbcDao() {
		super();
		initClass();
		initMetaFactory();
	...
	// Collection<Column<T>> supIdColumns =
	// getColumnsByFieldName(getSupIdFieldName());
	// Collection<Column<T>> pssColumns = new
	// ArrayList<Column<T>>(supIdColumns);
	//
	// String SQL = "DELETE FROM #{table} WHERE #{whereContents}";
	// SQL = sqlSetting(SQL, "table", table.getTableName());
	// SQL = sqlSetting(SQL, "whereContents",
	// getWhereContentsForSupId(supIdColumns));
	//
	// T object = produceObjectBySupId(supId);
	// jdbcTemplate.update(SQL, providePsSetter(pssColumns, object));
	// }

	public List<ID> findAllIdBySupId(SUPID supId) {
		Collection<Column<T>> idColumns = getColumnsByFieldName(getIdFieldName());
		...
		String SQL = "SELECT #{columnNames} FROM #{table} WHERE #{whereContents}";
		SQL = sqlSetting(SQL, "table", table.getTableName());
		SQL = sqlSetting(SQL, "columnNames", new SelectContents<T>(idColumns).toString());
		SQL = sqlSetting(SQL, "whereContents", getWhereContentsForSupId(supIdColumns));

		T object = produceObjectBySupId(supId);
		List<T> list = jdbcTemplate.query(SQL, providePsSetter(pssColumns, object), provideRowMapper(idColumns));

		List<ID> idList = new ArrayList<ID>();
		for (T objectWithId : list) {
			idList.add(fetchId(objectWithId));
		}
		return idList;
	}
